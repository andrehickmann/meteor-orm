...
/**
* The adapter can connect to any mysql-database and perform query's on it.
...
Mysql = ( function() {
   ...
   * @param {object} dbSettings for connection to the db
   * 
  ...
  */
   ...
   * @returns {String}
   */
  Mysql.prototype._escapeIdentifier = function(value) {
    return this.getConnection().escapeId(value);
  };

  /**
   * returning th e columns of the table.
   *
   * @param Orm.Db.Table table
   * @returns {*}
   */
  Mysql.prototype.getColumnsOfTable = function(table) {
    return this.query('SHOW COLUMNS FROM ' + this._escapeIdentifier(table.getName()));
  };
