...
let Select = (function(){
  ...
  Select.prototype._parseFrom = function(table) {
    ...
    fromString += this.WS + this.FROM + this.WS + table.getAdapter().escapeIdentifier(table.getName());
    return fromString;
  };
  ...
  Select.prototype._parseJoin = function(joins) {
    ...
    joins.forEach(function(join, index, values){
      ...
      joinString += join.type + this.WS +
                    currentTable + this.WS + this.ON + this.WS +
                    join.on;
  ...
  Select.prototype._parseWhere = function(whereParts) {
    ...
    if (whereParts.length > 0) {
      whereString += this.WS + this.WHERE + this.WS;
    }
    whereParts.forEach(function(orPart, orIndex, orArray) {
      ...
      orPart.forEach(function(andPart, andIndex, andArray){
        ...
        whereString += this.getTable().getAdapter().escapeIdentifier(andPart.table) + this.CONCAT +
                       this.getTable().getAdapter().escapeIdentifier(andPart.column) + this.WS +
                       condition + this.WS + this.getTable().getAdapter().escape(andPart.value);
        if (andIndex < (andArray.length - 1)) {
          whereString += this.WS + this.AND + this.WS;
        } 
      }, this);
      if (orPart.length > 1) {
        whereString += ')';
      ...
      if (orIndex < (orArray.length - 1)) {
        whereString += this.WS + this.OR + this.WS;
      }     
    }, this);
    return whereString;
  };
  ...
  Select.prototype.joinLeft = function(table, on, columns) {
    ...
    this.addJoin(this.JOINLEFT, table, on, columns);
    return this;
  };
