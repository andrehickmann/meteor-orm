...
Adapter = (function() {
  ...
  function Adapter(connSettings, name) {
    if (!connSettings) {
      throw new Error('You have to pass the connection settings.');
    }
    if (!name) {
      throw new Error('You have to define a name for the adapter.');
    }
  ...
  */
  ...
  /**
  * returns a boolean flag if the initialization was done.
  *
  * @return boolean
  */
  ...
  */
   ...
   * @returns {String}
   */
  Adapter.prototype.escapeIdentifier = function(value) {
    return this._escapeIdentifier(value);
  };
